import datetime  # 日付を扱うために追加
import streamlit as st

st.title("To do list+")
st.caption("タスクを記入してやること管理しよう！")
...
def _normalize_tasks() -> None:
    """すべてのタスクに一意のIDと日付（未設定なら今日）があることを保証。"""
    updated = False
    ...
    for item in st.session_state.todo_list:
        if "i" not in item:
            ...
            updated = True
        if "date" not in item:
            item["date"] = today  # 古いデータに日付がない場合は今日の日付を補完
            updated = True
    if updated:
...
st.sidebar.success(
    """
    このアプリは「カレンダー対応ToDoリスト」です。
    - 日付を選択してタスクを登録することが可能です。
